...
from tensorflow.keras.layers import Dense, Conv1D, GlobalMaxPooling1D, Input, concatenate
...
def load_dataset(dataset, filepath, identity_feature, negative_random_sample, use_side_feature, identity_feature_dim=1024):
    ...
    if use_side_feature:
        ...
        RNA_side_feature = pd.read_csv(os.path.join(filepath,'RNA4merfeat.csv')).values
...
def train(NPI_pos_matrix, NPI_neg_matrix, protein_identity_feature, RNA_identity_feature, protein_side_feature,
    ...
    for i in range(5):
        ...
        mask[test_data[i][:, 1], test_data[i][:, 2]] = 0 
        print(mask.sum())
    ...
    print("The final performance of RPI-MD is:")
...
def compare_different_achitectures(filepath, savepath, INI_PATH):
    print("compare_different_architectures is running")
